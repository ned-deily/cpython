        ...
        result.extend(
            [
                ...
                dict(
                    ...
                    patches=["tk868_on_10_8_10_9.patch"],
                    buildDir="unix",
                    configure_pre=[
                        "--enable-aqua",
                        "--enable-shared",
...
def buildPython():
    ...
    os.environ["DYLD_LIBRARY_PATH"] = os.path.join(
        WORKDIR, "libraries", "usr", "local", "lib"
    )
    ...
    runCommand("make install DESTDIR=%s" % (shellQuote(rootDir)))

    print("Running make frameworkinstallextras")
    runCommand("make frameworkinstallextras DESTDIR=%s" % (shellQuote(rootDir)))
...
def buildDMG():
    ...
    imagepath = os.path.join(
        outdir, "python-%s-macosx%s" % (getFullVersion(), DEPTARGET)
    )
